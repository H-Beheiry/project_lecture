...
#include "../../include/nodes/CARTproduct.h"
...
void staff::dequeCustomer()
{
    ...
    while (curr!=NULL)
    {
        if (!product_catalog->search(curr->data->productID))
        {
    ...
    string uname = first_customer->user->getUsername();
    first_customer->user->setStatus("BROWSING");
    checkout_line->dequeue();
    cout<<GREEN<<"Checkout completed for "<<uname<<RESET<<endl;
    }
